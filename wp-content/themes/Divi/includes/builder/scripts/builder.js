...
( function($) {
	...
	$( document ).ready( function() {
		...
		ET_PageBuilder.ModuleSettingsView = window.wp.Backbone.View.extend( {
			...
			render : function() {
				...
				_.each( this.model.attributes, function( value, key, list ) {
					if ( typeof value === 'string' && key !== 'et_pb_content_new' ) {
		...
		ET_PageBuilder.AppView = window.wp.Backbone.View.extend( {
			...
			saveAsShortcode : function( et_model, et_collection, et_options ) {
				...
				setTimeout( function(){
					...
					var action = action_setting !== 'load_layout' ? 'saving_to_content' : '';
		...
		function et_pb_set_content( textarea_id, content, current_action ) {
			var current_editor_mode = typeof window.switchEditors !== 'undefined' ? et_get_editor_mode() : '',
				is_in_visual_mode   = !! ( typeof window.tinyMCE !== 'undefined' && window.tinyMCE.get( textarea_id ) && ! window.tinyMCE.get( textarea_id ).isHidden() ),
				current_action      = current_action || '',
				save_to_content     = current_action === 'saving_to_content';

			// Apply wpautop to make sure formatting isn't lost.
			// Use wpautop, when a module settings is being rendered,
			// don't re-apply it, if we're saving data into the page content.
			if ( ! save_to_content && typeof window.switchEditors !== 'undefined' ) {
				content = window.switchEditors.wpautop( content );
			}

			if ( is_in_visual_mode && current_editor_mode === 'tinymce' ) {
